...
{
	win_env_t *e;
	const char *bg, *fg, *sel_col, *mar_col, *f;
	char *res_man;
	XrmDatabase db;
	...
	sel_col = win_res(db, RES_CLASS ".selectionColor", "white");
	mar_col = win_res(db, RES_CLASS ".markedColor", "white");
	win_alloc_color(e, bg, &win->bg);
	win_alloc_color(e, fg, &win->fg);
	win_alloc_color(e, sel_col, &win->sel_col);
	win_alloc_color(e, mar_col, &win->mar_col);
...
void win_draw_bar(win_t *win)
{
	...
	XSetForeground(e->dpy, gc, options->swap_bar ? win->bg.pixel : win->fg.pixel);
	XFillRectangle(e->dpy, win->buf.pm, gc, 0, win->h, win->w, win->bar.h);

	XSetForeground(e->dpy, gc, options->swap_bar ? win->fg.pixel : win->bg.pixel);
	XSetBackground(e->dpy, gc, options->swap_bar ? win->bg.pixel : win->fg.pixel);
	...
	if ((len = strlen(r->buf)) > 0) {
		...
		w -= tw;
		win_draw_text(win, d, options->swap_bar ? &win->fg : &win->bg, x, y, r->buf, len, tw);
	}
	if ((len = strlen(l->buf)) > 0) {
		x = H_TEXT_PAD;
		w -= 2 * H_TEXT_PAD; /* gap between left and right parts */
		win_draw_text(win, d, options->swap_bar ? &win->fg : &win->bg, x, y, l->buf, len, w);
	}
	XftDrawDestroy(d);
}
